...
{
// std::cout << std::endl;
// std::cout << "Starting Cascade" << std::endl;
...
{ 
  for (int i = 0; i < e->out.size(); i++)
  {
    ...
    //std::cout << "Borrowing energy: " << p1.pdg << "   " << nucl->hyp_BE(p1.r.length(),p1.pdg) << std::endl;
...
bool kaskada::leave_nucleus()
{
  ...
  else if(hyperon(p->pdg))
  {
    ...
    // subtract binding energy from hyperon energy and set momentum so it is on shell
	    ...
	    // std::cout << "Returning energy: " << p->pdg << "  " << p->his_fermi << std::endl;
	    //p->set_fermi(-100);
    }
...
/* KN: Is it important to keep the lines above? */
  ...
  for (int i = 0; i < X.n; i++)
	...
	//TODO check this
	//try with this, what is the effect?
...
bool kaskada::finalize_interaction()
{
  ...
  for (int i = 0; i < X.n; i++)
  {
	...
	//if hyperon switched families, move to new potential
	if( (PDG::Lambda(p->pdg) && PDG::Sigma(X.p[i].pdg)) || (PDG::Sigma(p->pdg) && PDG::Lambda(X.p[i].pdg)) ){

	//for debuging/testing -> turn of potential swtiching feature
	//if(true){
	//if(false){

	X.p[i].set_fermi(nucl->hyp_BE(X.p[i].r.length(),X.p[i].pdg));

	//if cannot move to new position in potential, jail
	if(X.p[i].Ek() < p->his_fermi - nucl->hyp_BE(X.p[i].r.length(),X.p[i].pdg)){
			X.p[i].endproc=jailed;
			if(par.kaskada_writeall) 
			e->all.push_back(X.p[i]);
			continue;
	...
	//std::cout << "Returning energy: " << p->pdg << "  " << p->his_fermi << std::endl;
	//std::cout << "Borrowing energy: " << X.p[i].pdg << "  " << nucl->hyp_BE(X.p[i].r.length(),X.p[i].pdg) << std::endl; 
 
	//move to new potential	
	X.p[i].set_energy(X.p[i].E() - p->his_fermi + nucl->hyp_BE(X.p[i].r.length(),X.p[i].pdg));
		
	}
	//if hyperon does not switch families, copy potential to outgoing particle
	else	{    
	
		X.p[i].set_fermi(p->his_fermi);

		}//if hyperon did not swtich families
